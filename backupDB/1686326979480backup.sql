...
INSERT INTO
  `carspareparts` (`id`, `name`, `price`, `partNumber`)
VALUES
  (53, 'Бампер Toyota Camry', 2000.40, '123455533N234');
...
CREATE TRIGGER `assign_mechanic_on_insert` BEFORE INSERT ON `orders` FOR EACH ROW BEGIN
    ...
    -- Визначення механіка з найменшою кількістю поточних замовлень
    SET mechanic_id = (
        SELECT id FROM (
    ...
    IF mechanic_id IS NULL THEN
        -- Якщо всі механіки мають 3 або більше поточних замовлень
        SET NEW.status = 'Очікує';
        SET NEW.workerId = NULL;
    ELSE
        -- Якщо знайдено механіка з менше ніж 3 поточними замовленнями
