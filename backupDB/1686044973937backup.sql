...
# ------------------------------------------------------------
# TRIGGER DUMP FOR: check_if_car_is_already_on_repair_before_insert_orders
# ------------------------------------------------------------

DROP TRIGGER IF EXISTS check_if_car_is_already_on_repair_before_insert_orders;
DELIMITER ;;
CREATE TRIGGER `check_if_car_is_already_on_repair_before_insert_orders` BEFORE INSERT ON `orders` FOR EACH ROW BEGIN
    DECLARE carCount INT;
    
    SELECT COUNT(*) INTO carCount
    FROM orders
    WHERE carId = NEW.carId
        AND (status <> 'виконано' OR issueDate IS NULL);
        
    IF carCount > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'This car is repairing at the moment';
    END IF;
END;;
DELIMITER ;
